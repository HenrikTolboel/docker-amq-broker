...
function injectGracefulShutdown() {
  instanceDir=$1
  echo "Adding graceful shutdown"
  sed -i "s/^\([[:blank:]]*\)<\\/core>/\1\1<graceful-shutdown-enabled>true<\\/graceful-shutdown-enabled>\\n\1\1<graceful-shutdown-timeout>5000<\\/graceful-shutdown-timeout>\\n\1<\\/core>/" $instanceDir/etc/broker.xml
}
...
function configure() {
  ...
  if [ ! -d ${instanceDir} -o "$AMQ_RESET_CONFIG" = "true" -o ! -f ${instanceDir}/bin/artemis ]; then
    ...
    if [ "$AMQ_ENABLE_GRACEFUL_SHUTDOWN" = "true" ]; then
      echo "Enable graceful shutdown"
      injectGracefulShutdown ${instanceDir}
    fi
